procedure changecalnames(inimages)
#,flat)
...
#OBJECT  = 'hd124740'
        if (substr(line,1,6) == "OBJECT"){ 
          ...
          if (substr(line,16,19) == "WAVE")
            out=out//"ThAr"
          else if (substr(line,12,15) == "BIAS")
            out=out//"Zero"
          else{
...
#DATE-OBS= '2002-01-25T09:25:45.243'
        if (substr(line,1,8)=="DATE-OBS"){
          outdate="_CES"//substr(line,12,34)
        }
#HIERARCH ESO INS GRAT1 WLEN  =  568.4000000
        else if (substr(out,5,8) != "Zero" && substr(line,1,27) == "HIERARCH ESO INS GRAT1 WLEN"){
          if (substr(line,33,37) == "568.4")
            out = out//"_6145_"
          else
            out = out//"_"//substr(line,33,35)//substr(line,37,37)//"_"
          print("out = "//out)          
        }
#HIERARCH ESO DET WIN1 UIT1   =    60.000000
        else if (substr(out,5,8) != "Zero" && substr(line,1,26) == "HIERARCH ESO DET WIN1 UIT1"){
