    ...
    /// string input_fits_file_name: in, string input_database_file_name: in, string output_fits_file_name: out, string output_blaze_file_name: out, double Gain: in, double ReadOutNoise: in, int LambdaSP: in, double MinSNR: in[, int SwathWidth: in]
{
  ...
  if (argc < 10)
  {
    ...
    cout << "USAGE: makenormflat <char[] FitsFileName_In> <char[] DatabaseFileName_In> <char[] NormalisedFlat_Out> <char[] ProfileFits_Out> <char[] BlazeFits_Out> <double Gain> <double ReadOutNoise> <int SmoothSP> <double MinSNR> [<int SwathWidth>][SMOOTH_SF=<double smooth_slit_func>] [AREA=[<int xmin>,<int xmax>,<int ymin>,<int ymax>]] [MASK_OUT=<char[] Mask_Out>] [RECONSTRUCTED_IMAGE_OUT=<char[] ReconstructedImage_Out>]" << endl;
    cout << "Parameter 1: <char[] FitsFileName_In>: input flat fits file name" << endl;
    cout << "Parameter 2: <char[] DatabaseFileName_In>: input Database file name" << endl;
    cout << "Parameter 3: <char[] NormalisedFlat_Out>: output fits file name" << endl;
    cout << "Parameter 4: <char[] ProfileFits_Out>: output profile image fits file name" << endl;
    cout << "Parameter 5: <char[] BlazeFits_Out>: output blaze functions fits file name" << endl;
    cout << "Parameter 6: <double Gain>: CCD gain" << endl;
    cout << "Parameter 7: <double ReadOutNoise>: CCD read-out noise" << endl;
    cout << "Parameter 8: <int SmoothSP>: smooth blaze function over SmoothSP pixels" << endl;
    cout << "Parameter 9: <double MinSNR>: minimum S/N to use for each pixel of the normalised flat. Pixels with lower S/N are set to unity" << endl;
    cout << "Parameter 10: [<int SwathWidth>]: optional, length of swath to be used for the determination of the spatial profile" << endl;
    cout << "Parameter 11: [SMOOTH_SF=<double smooth_slit_func>]: optional, smoothing factor for the spatial profile (default = 1/10 with 10 pixels oversampling)" << endl;
    cout << "Parameter 12: [AREA=[<int xmin>,<int xmax>,<int ymin>,<int ymax>]]: optional, area on the CCD to calculate and remove scattered light" << endl;
    cout << "Parameter 13: [MASK_OUT=<char[] Mask_Out>]: optional, output fits file containing the mask for the detected bad-pixels / cosmic-rays" << endl;
    cout << "Parameter 14: [RECONSTRUCTED_IMAGE_OUT=<char[] ReconstructedImage_Out>]: optional, output fits file containing the reconstructed flat" << endl;
    cout << "example: makenormflat flat.fits database/apflat normalizedFlat.fits flat_prof.fits flat_blaze.fits 3.8 10.5 5 100 300 SMOOTH_SF=0.5 MASK_OUT=flat_mask.fits RECONSTRUCTED_IMAGE_OUT=flat_rec.fits" << endl;
    exit(EXIT_FAILURE);
  }
