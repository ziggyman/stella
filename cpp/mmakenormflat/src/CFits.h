...
//    #define __DEBUG_FITS_ESTIMATESCATTEREDLIGHT__
...
//    #define __DEBUG_FITS_SLITFUNC_PISKUNOX__
  ...
  class CFits: public CAny
  {
    ...
    public:
      ...
      **/
      ...
      Array<double, 2>* Get_XCentersPointer();/// NApertures x NRows
...
//                           Array<double, 1> &D_A1_Coeffs_Out) const;
      ...
      bool FindAndTraceApertures(const int I_NApertures_In,
...
//                                                   Array<double, 1> &D_A1_Out,
      ...
      **/
      ...
      bool Set_ApertureDataToZero(const int I_PlusNPixels_X, 
                                  const int I_PlusNPixels_Y);
      bool Set_ApertureDataToZero(Array<double, 2> &D_A2_InOut, 
                                  const int I_PlusNPixels_X, 
                                  const int I_PlusNPixels_Y) const;
...
//                             void *PP_Args_In[]) const;
      ...
      bool EstScatterKriging(const Array<double, 2> &D_A2_PixArray_In,
                             ...
                             void *PP_Args_In[]) const;
                             /// AREA = Array<int, 1>(4) 0: I_XMin, 1: I_XMax, 2: I_YMin, 3: I_YMax
                             /// MEDIAN = 1: Take median value of cluster for scattered light, otherwise
                             ///             (minimum value + 3 RDNoise)


      bool EstimateScatteredLight(const Array<double, 2> &D_A2_PixArray_In,
                                  const int I_NRectangles_X,
                                  const int I_NRectangles_Y,
                                  Array<double, 2> &D_A2_ScatteredLight_Out,
                                  Array<CString, 1> &CS_A1_Args_In,
                                  ...
                                  /// MEDIAN = 1: Take median value of cluster for scattered light, otherwise
                                  ///             (minimum value + 3 RDNoise)
                                  /// METHOD: CString("Kriging") (default) or CString("BiSpline")
                                  /// if METHOD != Kriging: SMOOTH: double >= 0; default = 1e-16
                                  /// if METHOD != Kriging: IOPT: int[-1,0]; default = -1
                                  ///                                 -1: least squares fit
                                  ///                                 0: smoothing spline
                                  /// if METHOD != Kriging: See SurfaceFit.h
                                  
      /* See SurfaceFit.h
      bool CalculateScatteredLightBiSpline(const Array<double, 2> &D_A2_PixArray_In,
