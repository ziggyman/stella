...
int main(int argc, char *argv[])
{
  ...
  Array<double, 1> D_A1_Y_Gauss(10);
  D_A1_Y_Gauss(0) = 0.;
  D_A1_Y_Gauss(1) = 0.1;
  D_A1_Y_Gauss(2) = 0.6;
  D_A1_Y_Gauss(3) = 2.1;
  ...
  D_A1_Y_Gauss(8) = 0.1;
  D_A1_Y_Gauss(9) = 0.0;
  cout << "D_A1_Y_Gauss = " << D_A1_Y_Gauss << endl;

  Array<double, 1> D_A1_Y_DGauss(10);
  D_A1_Y_DGauss(0) = 0.;
  D_A1_Y_DGauss(1) = 1.;
  D_A1_Y_DGauss(2) = 2.6;
  D_A1_Y_DGauss(3) = 3.1;
  ...
  D_A1_Y_DGauss(8) = 2.1;
  D_A1_Y_DGauss(9) = 0.2;
  cout << "D_A1_Y_DGauss = " << D_A1_Y_DGauss << endl;
  ...
  f->execute("guess %dgauss = Gaussian");
  f->execute("fit");
  cout << "peak center: " << f->calculate_expr("%dgauss.Center") << endl;
  delete f;

  f = new Fityk;
  for (int i=0; i<D_A1_X.size(); i++){
    f->add_point(D_A1_X(i), D_A1_Y_Gauss(i), sqrt(D_A1_Y_Gauss(i)));
  }
//  f->execute("Y = randnormal(y, s)");
...
/**
  ...
  if (!CF.InterPol(D_A1_Y, D_A1_X, D_A1_U, &D_A1_Out, true)){
    cout << "MExtractSum_Obs_Sky::main: ERROR: 1. InterPol returned FALSE" << endl;
    exit(EXIT_FAILURE);
  }
