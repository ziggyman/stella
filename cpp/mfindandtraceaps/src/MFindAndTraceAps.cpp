...
int main(int argc, char *argv[])
{
  ...
  if (argc < 19)
  {
    ...
    cout << "Parameter 16: int ExtendAps_Up - number of pixels to extend a trace upwards - set to 0 for no extension," << endl;
    cout << "Parameter 17: int ApertureLength_Down - aperture length downwards after applying ExtendAps_Up - set to 0 for no extension," << endl;
    ...
    cout << "Parameter 23: [FitsFileName_ThresholdZero_Out=(char[] FitsFileName_ThresholdZero_Out) - Quality control file with all pixels lower than SignalThreshold_In set to zero]" << endl;
    exit(EXIT_FAILURE);
  }
  ...
  CString CS_FitsFileName_ThresholdZero_Out("");
...
//  char *P_CharArr_Out = (char*)argv[16];
  ...
  for (int i = 19; i <= argc; i++){
    ...
    CS_comp.Set("FitsFileName_ThresholdZero_Out");
    if (CS.GetLength() > CS_comp.GetLength()){
      delete(P_CS);
      P_CS = CS.SubString(0,CS.CharPos('=')-1);
      cout << "MFindAndTraceAps::main: *P_CS set to " << *P_CS << endl;
      if (P_CS->EqualValue(CS_comp)){
        delete(P_CS);
        P_CS = CS.SubString(CS_comp.GetLength()+1);
        CS_FitsFileName_ThresholdZero_Out = P_CS->Get();
        cout << "MFindAndTraceAps::main: CS_FitsFileName_ThresholdZero_Out set to " << CS_FitsFileName_ThresholdZero_Out << endl;
      }
    }
  }
...
//  D_A2_Temp.resizeAndPreserve(D_A2_Flat.rows(), D_A2_Flat.cols());
  ...
  std::string fitsFileName_ThresholdZero_Out(CS_FitsFileName_ThresholdZero_Out.GetPChar());
  if (!F_Image.FindAndTraceApertures(I_NTermsGaussFit_In,
                                     I_PolyFitOrder_In,
				     I_MinLength_In,
				     I_MaxLength_In,
                                     I_NLost_In,
                                     fitsFileName_ThresholdZero_Out)){
